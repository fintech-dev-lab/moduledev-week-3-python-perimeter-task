...
Готовность сдачи включает первый запуск на пустой тестовой БД, актуальный README,
объявленную конфигурацию и команду проверки текущей недели. Необъявленный повтор запуска
или ручное исправление окружения не заменяют этот результат. Ранее доставленные замечания
проверяются повторно по существу; исправления и подтверждённые повторы отражаются в отчёте.
